...
const DiffMatchPatch = require("diff-match-patch")//그림퀴즈
...
const differ = new DiffMatchPatch()//그림퀴즈
...
exports.Client = function(socket, profile, sid){
	...
	socket.on('message', function(msg){
		...
		//JLog.log(`Chan @${channel} Msg #${my.id}: ${msg}`); 그림퀴즈
		try{ data = JSON.parse(msg); }catch(e){ data = { error: 400 }; }
		JLog.log(`Chan @${channel} Msg #${my.id}: ${data.type == 'drawingCanvas' ? JSON.stringify({type: data.type, diffed: data.diffed}) : msg}`); //그림퀴즈
	...
	*/
	my.drawingCanvas = function(msg) {//그림퀴즈 추가 시작
...
exports.Room = function(room, channel){
	...
	my.drawingCanvas = function(msg, userid) { //msg -> Message, userid -> sender ID 그림퀴즈
